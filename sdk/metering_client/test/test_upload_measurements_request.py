# coding: utf-8

"""
    Metering

    MeterData API

    The version of the OpenAPI document: 1.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from metering_client.models.upload_measurements_request import UploadMeasurementsRequest

class TestUploadMeasurementsRequest(unittest.TestCase):
    """UploadMeasurementsRequest unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> UploadMeasurementsRequest:
        """Test UploadMeasurementsRequest
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `UploadMeasurementsRequest`
        """
        model = UploadMeasurementsRequest()
        if include_optional:
            return UploadMeasurementsRequest(
                automatic_processing = True,
                source = '',
                upload_identifier = '',
                channel_measurements = [
                    metering_client.models.channel_measurement_request.ChannelMeasurementRequest(
                        utility_type = null, 
                        unit_of_measure = null, 
                        metering_type = null, 
                        direction = null, 
                        meter_serial_number = '', 
                        external_channel_identifier = '', 
                        measurements = [
                            metering_client.models.measurement_request.MeasurementRequest(
                                reading_date_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                                value = 1.337, )
                            ], )
                    ]
            )
        else:
            return UploadMeasurementsRequest(
        )
        """

    def testUploadMeasurementsRequest(self):
        """Test UploadMeasurementsRequest"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
