# coding: utf-8

"""
    Masterdata

    MasterData API

    The version of the OpenAPI document: 1.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from masterdata_client.models.create_meter_request import CreateMeterRequest

class TestCreateMeterRequest(unittest.TestCase):
    """CreateMeterRequest unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> CreateMeterRequest:
        """Test CreateMeterRequest
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `CreateMeterRequest`
        """
        model = CreateMeterRequest()
        if include_optional:
            return CreateMeterRequest(
                serial_number = '',
                meter_type = 'individual',
                meter_tag = '',
                model_id = '',
                mutation_date_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                channels = [
                    masterdata_client.models.external_channel_request.ExternalChannelRequest(
                        data_frequency = null, 
                        metering_type = null, 
                        utility_type = null, 
                        direction = null, 
                        incrementation_type = null, 
                        time_of_use = '', 
                        unit_of_measure = null, 
                        external_identifier = '', 
                        description = '', 
                        relation_external_reference = '', )
                    ],
                reading_frequency = 'hourly',
                next_expected_reading_date = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                properties = None,
                parent_relation = masterdata_client.models.create_meter_parent_relation_request.CreateMeterParentRelationRequest(
                    parent_meter_id = '', 
                    external_reference = '', )
            )
        else:
            return CreateMeterRequest(
        )
        """

    def testCreateMeterRequest(self):
        """Test CreateMeterRequest"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
