...
class SettlementDetailsDTO(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in settled_transactions_with_amounts (list)
        _items = []
        if self.settled_transactions_with_amounts:
            for _item_settled_transactions_with_amounts in self.settled_transactions_with_amounts:
                if _item_settled_transactions_with_amounts:
                    _items.append(_item_settled_transactions_with_amounts.to_dict())
            _dict['settledTransactionsWithAmounts'] = _items
        # set to None if settled_transactions_with_amounts (nullable) is None
        # and model_fields_set contains the field
        if self.settled_transactions_with_amounts is None and "settled_transactions_with_amounts" in self.model_fields_set:
            _dict['settledTransactionsWithAmounts'] = None
