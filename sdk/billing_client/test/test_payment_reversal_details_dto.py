# coding: utf-8

"""
    Billing

    Billing API

    The version of the OpenAPI document: 1.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from billing_client.models.payment_reversal_details_dto import PaymentReversalDetailsDTO

class TestPaymentReversalDetailsDTO(unittest.TestCase):
    """PaymentReversalDetailsDTO unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> PaymentReversalDetailsDTO:
        """Test PaymentReversalDetailsDTO
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `PaymentReversalDetailsDTO`
        """
        model = PaymentReversalDetailsDTO()
        if include_optional:
            return PaymentReversalDetailsDTO(
                reversal_reason = 'manualreverse',
                reversal_additional_information = '',
                reversal_date = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                reversal_incoming_banking_transaction_id = '',
                reversal_incoming_banking_transaction_identification = '',
                reversal_incoming_mutation_id = ''
            )
        else:
            return PaymentReversalDetailsDTO(
        )
        """

    def testPaymentReversalDetailsDTO(self):
        """Test PaymentReversalDetailsDTO"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
