...
class CreateCostUnitsRequest(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in cost_units (list)
        _items = []
        if self.cost_units:
            for _item_cost_units in self.cost_units:
                if _item_cost_units:
                    _items.append(_item_cost_units.to_dict())
            _dict['costUnits'] = _items
        # set to None if cost_units (nullable) is None
        # and model_fields_set contains the field
        if self.cost_units is None and "cost_units" in self.model_fields_set:
            _dict['costUnits'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "costUnits": [CreateCostUnitRequest.from_dict(_item) for _item in obj["costUnits"]] if obj.get("costUnits") is not None else None
        })
        return _obj
