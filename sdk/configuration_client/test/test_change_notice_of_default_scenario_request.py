# coding: utf-8

"""
    Configuration

    Configuration API

    The version of the OpenAPI document: 1.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from configuration_client.models.change_notice_of_default_scenario_request import ChangeNoticeOfDefaultScenarioRequest

class TestChangeNoticeOfDefaultScenarioRequest(unittest.TestCase):
    """ChangeNoticeOfDefaultScenarioRequest unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ChangeNoticeOfDefaultScenarioRequest:
        """Test ChangeNoticeOfDefaultScenarioRequest
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ChangeNoticeOfDefaultScenarioRequest`
        """
        model = ChangeNoticeOfDefaultScenarioRequest()
        if include_optional:
            return ChangeNoticeOfDefaultScenarioRequest(
                pdf_template = configuration_client.models.notice_of_default_pdf_template_request.NoticeOfDefaultPdfTemplateRequest(
                    envelope_settings = null, 
                    show_country = True, ),
                email_template = configuration_client.models.notice_of_default_email_template_request.NoticeOfDefaultEmailTemplateRequest(
                    subject = '', 
                    attachments = [
                        configuration_client.models.template_attachment_request.TemplateAttachmentRequest(
                            id = '', 
                            internal_file_path = '', 
                            file_name = '', )
                        ], )
            )
        else:
            return ChangeNoticeOfDefaultScenarioRequest(
        )
        """

    def testChangeNoticeOfDefaultScenarioRequest(self):
        """Test ChangeNoticeOfDefaultScenarioRequest"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
